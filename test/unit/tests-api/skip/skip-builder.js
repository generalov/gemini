...
describe('tests-api/skip/skip-builder', () => {
    ...
    describe('in', () => {
        ...
        it('should throw if argument is array with non-string or non-RegExp', () => {
            assert.throws(() => {
                skipBuilder.in([false]);
            }, TypeError, errorMessage);
        });

        it('should throw if argument is an object', () => {
            assert.throws(() => {
                skipBuilder.in({browserName: 'name', version: '123', id: 'browser'});
            }, TypeError, errorMessage);
        });
    ...
    describe('notIn', () => {
        ...
        it('should throw if argument is array with non-string or non-RegExp', () => {
            assert.throws(() => {
                skipBuilder.notIn([false]);
            }, TypeError, errorMessage);
        });

        it('should throw if argument is an object', () => {
            assert.throws(() => {
                skipBuilder.notIn({browserName: 'name', version: '123', id: 'browser'});
            }, TypeError, errorMessage);
        });
