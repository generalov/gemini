...
module.exports = class SuiteBuilder {
    ...
    setCaptureElements(...selectors) {
        ...
        if (!selectors.every(_.isString)) {
            throw new TypeError('this._suite.captureElements accepts only strings or array of strings');
        }
    ...
    capture(name, opts, cb) {
        ...
        if (!_.isFunction(cb)) {
            throw new TypeError('Second argument of this._suite.capture must be a function');
        }

        if (this._suite.hasStateNamed(name)) {
            throw new Error(`State "${name}" already exists in this._suite "${this._suite.name}". Choose different name`);
        }
    ...
    _injectSkip() {
        const skipBuilder = new SkipBuilder(this._suite);
        const onlyBuilder = new OnlyBuilder(this._suite);

        _.extend(this, skipBuilder.buildAPI(this));
        _.extend(this, onlyBuilder.buildAPI(this));
    }
